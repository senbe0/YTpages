...
const Menu = ({ group_name }) => {
  const renderMenuItem = (label, link, isActive) => {
    if (isActive) {
      return <li className="menu">{label}</li>;
    } else {
      return (
        <li className="menu">
          <Link href={link} className="can-selectMenu">
            {label}
          </Link>
        </li>
      );
  ...
  return (
    <nav>
      <ul className="menubar">
        {renderMenuItem("Home", "/", group_name === undefined)}
        {renderMenuItem("JP", "/group/jp", group_name === "jp")}
        {renderMenuItem("EN", "/group/en", group_name === "en")}
        {renderMenuItem("ID", "/group/id", group_name === "id")}
        {renderMenuItem("Info", "/info", false)}
      </ul>
    </nav>
  );
};
